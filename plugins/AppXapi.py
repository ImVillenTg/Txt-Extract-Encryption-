...
async def start(bot, m):
    ...
    data = {
        'email': '',
        'password': '',
        'devicetoken': 'evxVp-BBB3I:APA91bFSglfbsDx7kYeVNnOszxud1cUyXj-p54ejyaSvItmM7p5EPH9iyZKKk0N66gROVI3cRWVg1Bvy4tuBsU1VPulrjKqoiF644NI9dqKUswrnOc5TLd0ZHrTZsgy6tSLpcG6OMz7A',
            ...
            for data in gg:
                if ((data["material_type"]) != "VIDEO") and ((data["file_link"]) != ""):
                    ...
                    if pdf_link and (pdf_link != file_link):
                           video_link += f'\nPDF {title.replace(":","")}: {pdf_link}'
                    if pdf_link2:
                        video_link += f'\nPDF-2 {title.replace(":","")}: {pdf_link2}'
