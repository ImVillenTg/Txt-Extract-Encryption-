...
def process_folder(api, bid, fid, fname, f):
    cdatas = fapi2(api, bid, fid)
    for cdata in cdatas:
        ...
        if mtype == "IMAGE":
            ...
            vid = cdata["id"]
            title, vlink, plink1, plink2 = vapi(api, bid, vid)
            ...
            fid = cdata["id"]
            fname = cdata["Title"]
            process_folder(api, bid, fid, fname, f)
...
async def start(bot, m):
    ...
    for data in html1:
        if data["id"] == raw_text1:
            ...
            fid, fname = fapi1(api, course_id)
            with open(f"{course_name}.txt", "w") as f:
                process_folder(api, course_id, fid, fname, f)
